...
1.  If you are running locally instead of [this RStudio cloud workspace](https://rstudio.cloud/project/1430828/), install the [`targets`](https://github.com/wlandau/targets) package, as well as the packages listed in the `tar_options()` call in `_targets.R`.
1. Install Keras and TensorFlow using `keras::install_keras()`. Run `tensorflow::tf_config()` to see if TensorFlow installed correctly.
